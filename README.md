# Latent Archon — Government Capability Briefs

Sales and outreach materials for federal agencies, DoD service branches, and commercial customers.

## Contents

| Document | Audience | Description |
|----------|----------|-------------|
| [capability-brief.md](capability-brief.md) | All federal agencies | Master capability brief — product overview, security posture, use cases |
| [brief-dod-army.md](brief-dod-army.md) | U.S. Army | Tailored for Army acquisition, logistics, and program offices |
| [brief-dod-navy.md](brief-dod-navy.md) | U.S. Navy / USMC | Tailored for NAVAIR, NAVSEA, MARCORSYSCOM, and fleet logistics |
| [brief-dod-airforce.md](brief-dod-airforce.md) | U.S. Air Force / Space Force | Tailored for AFLCMC, SpRCO, and space acquisition |
| [brief-intel.md](brief-intel.md) | Intelligence Community | Tailored for IC agencies working with CUI/FOUO analytical products |
| [brief-civilian.md](brief-civilian.md) | Civilian agencies (GSA, VA, HHS, etc.) | Tailored for civilian use cases — grants, benefits, regulatory docs |
| [brief-law-enforcement.md](brief-law-enforcement.md) | Law enforcement (federal, state, local) | Tailored for LE — case files, policy search, CJIS compliance |
| [deployment-tiers.md](deployment-tiers.md) | Technical / all audiences | Multi-tier architecture: fed, gov, cloud — isolation, IL levels, upgrade path |
| [fedramp-outreach-email.md](fedramp-outreach-email.md) | FedRAMP PMO | Draft email to 20x@fedramp.gov expressing readiness for Phase 2/3 |

## Usage

These are markdown documents designed to be:
1. Read directly on GitHub by government evaluators
2. Converted to PDF via the `publish-pdfs` workflow (coming soon)
3. Attached to RFI/RFQ responses and capability statements

## Classification

**UNCLASSIFIED** — No CUI, no FOUO. Safe for public distribution.
